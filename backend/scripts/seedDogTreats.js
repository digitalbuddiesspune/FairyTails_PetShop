...
const inputPath =
  '/Users/prajwalkamdi/Desktop/fairytales/FairyTails_PetShop/json/dog_treats (1).json';
const DEFAULT_IMAGE =
...
const parseCapacity = (name, baseUnit) => {
  if (typeof name === 'string') {
    ...
    const quantityMatch = normalized.match(/^(\d+(?:\+\d+)?(?:\.\d+)?)\s*(kg|g|gm|grams?|ml|l|ltr|litre|litres?|pieces?|pcs?)\b/i);
    if (quantityMatch) {
      ...
      return `${quantityMatch[1]}${unit === 'pieces' ? ' pieces' : unit}`;
    }
  }
...
const run = async () => {
  try {
    ...
    const deleted = await Food.deleteMany({ category: 'Dog', subCategory: 'Treats' });
    const inserted = await Food.insertMany(documents);

    console.log(`Deleted existing Dog Treats: ${deleted.deletedCount}`);
    console.log(`Seeded Dog Treats records: ${inserted.length}`);
    process.exit(0);
  } catch (error) {
    console.error('Dog Treats seed failed:', error.message);
    process.exit(1);
  }
};
