...
const inputPath =
  '/Users/prajwalkamdi/Desktop/fairytales/FairyTails_PetShop/json/fairy_tails_items_DRY DOG FOOD.json';
const DEFAULT_IMAGE =
...
const parseCapacity = (name, baseUnit) => {
  if (typeof name === 'string') {
    ...
    const comboMatch = normalized.match(/^(\d+(?:\.\d+)?(?:\+\d+(?:\.\d+)?)?)\s*(kg|g|gm|grams?|ml|l|ltr|litre|litres?)\b/i);
    if (comboMatch) {
      let unit = comboMatch[2].toLowerCase();
      if (unit === 'gm' || unit === 'gram' || unit === 'grams') unit = 'g';
      if (unit === 'ltr' || unit === 'litre' || unit === 'litres') unit = 'l';
      return `${comboMatch[1]}${unit}`;
    }
  }
...
const run = async () => {
  try {
    ...
    const deleted = await Food.deleteMany({ category: 'Dog', subCategory: 'Dry Food' });
    const inserted = await Food.insertMany(documents);

    console.log(`Deleted existing Dog Dry Food: ${deleted.deletedCount}`);
    console.log(`Seeded Dog Dry Food records: ${inserted.length}`);
    process.exit(0);
  } catch (error) {
    console.error('Dog Dry Food seed failed:', error.message);
    process.exit(1);
  }
};
