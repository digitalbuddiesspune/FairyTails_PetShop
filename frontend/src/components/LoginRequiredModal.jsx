  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl animate-popIn" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className={`${type.h3} text-gray-900 mb-3`}>Login Required</h2>
        <p className={`text-gray-600 ${type.bodySm} mb-6`}>{message}</p>
        <button
          onClick={handleOk}
          className={`w-full bg-[#2f5a87] hover:bg-[#1a4a7a] text-white font-bold py-3 rounded-xl transition-all ${type.button}`}
        >
          OK
        </button>
      </div>
