    ...
    const renderDeleteConfirm = () => (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
            <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl text-center">
                ...
                <p className={`${type.bodySm} text-gray-500 mb-6`}>This will permanently delete <span className={`${type.label} text-gray-700`}>"{deleteConfirm?.productName || deleteConfirm?.name}"</span>.</p>
                <div className="flex gap-3">
                    ...
                    <button onClick={() => handleDeleteProduct(deleteConfirm._id)} className={`${type.bodySm} flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl transition-colors`}>
                        Delete
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="animate-fadeIn">
            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h2 className={`${type.h2} text-gray-900`}>{categoryData.label}</h2>
                    <p className={`${type.bodySm} text-gray-500`}>Manage your {categoryData.label.toLowerCase()} inventory</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-20 bg-gray-50 rounded-2xl border border-gray-200">
                    <span className={`${type.hero} block mb-2`}>📦</span>
                ...
                <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th className={`${type.captionMedium} px-6 py-4 text-gray-500 uppercase`}>Product</th>
                                <th className={`${type.captionMedium} px-6 py-4 text-gray-500 uppercase`}>Category</th>
                                <th className={`${type.captionMedium} px-6 py-4 text-gray-500 uppercase`}>Details</th>
                                <th className={`${type.captionMedium} px-6 py-4 text-gray-500 uppercase`}>Price</th>
                                <th className={`${type.captionMedium} px-6 py-4 text-gray-500 uppercase text-right`}>Actions</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={product._id} className="hover:bg-gray-50 transition-colors group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <p className={`${type.bodySm} text-gray-900 line-clamp-1`}>{product.productName || product.name}</p>
                                                <p className={`${type.caption} text-gray-500`}>{product.brand}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`${type.captionMedium} inline-flex items-center px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700`}>
                                            {product.subCategory || product.category}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <p className={`${type.bodySm} text-gray-600`}>{getExtraInfo(product)}</p>
                                    </td>
                                    <td className="px-6 py-4">
                                        <p className={`${type.button} text-gray-900`}>{getDisplayPrice(product)}</p>
                                    </td>
